...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-full bg-linear-to-br from-[#0A0A0A] via-black to-[#0A0A0A] text-(--gold-primary) py-10 px-4 md:px-6 flex flex-col items-center">

      <div className="max-w-3xl w-full bg-black/40 border border-(--gold-primary)/20 rounded-2xl p-6 md:p-10 shadow-xl backdrop-blur-md space-y-10">
        ...
        <div className="flex justify-between items-center">
          ...
          {isOwner ? (
            <Link
              href="/profile/edit"
              className="px-5 py-2 rounded-xl bg-(--gold-primary) text-black font-bold hover:bg-(--gold-light) transition"
            >
              ✏️ Edit Profile
            </Link>
          ) : (
            <>
              {isFollowing ? (
                ...
                <button
                  onClick={follow}
                  className="px-5 py-2 rounded-xl bg-(--gold-primary) text-black font-bold hover:bg-(--gold-light)"
                >
                  Follow
                </button>
              )}
        ...
        <div className="flex items-center gap-6">
          <div className="w-20 h-20 rounded-full bg-[#1A1A1A] border border-(--gold-primary) flex items-center justify-center text-3xl font-bold">
        ...
        <div className="flex gap-10 text-center mt-4">
          <div className="flex flex-col bg-(--dark-base)/30 px-5 py-3 rounded-xl border border-(--gold-primary)/20 w-32">
            <span className="text-2xl font-bold text-(--gold-primary)">{followersCount}</span>
            <span className="opacity-70 text-sm">Followers</span>
          </div>

          <div className="flex flex-col bg-(--dark-base)/30 px-5 py-3 rounded-xl border border-(--gold-primary)/20 w-32">
            <span className="text-2xl font-bold text-(--gold-primary)">{followingCount}</span>
        ...
        <div className="space-y-4 text-lg leading-relaxed">
          ...
          <div className="flex items-center gap-3">
            ...
            Age: <span className="text-(--gold-primary)">{data.age || "N/A"}</span>
          </div>

          <div className="flex items-center gap-3">
            <Image src="/gender.png" width={20} height={20} alt="gender icon" />
            Gender: <span className="text-(--gold-primary)">{data.gender || "N/A"}</span>
          </div>

          <div className="flex items-center gap-3">
            <Image src="/location.png" width={20} height={20} alt="location icon" />
            Location: <span className="text-(--gold-primary)">{data.location || "N/A"}</span>
          </div>

          <div className="flex items-center gap-3">
            <Image src="/bio.png" width={20} height={20} alt="bio icon" />
            Bio: <span className="text-(--gold-primary)">{data.bio || "No bio added"}</span>
          </div>
